...
class DaoAnalyzerRunRepository(private val db: Database) : AnalyzerRunRepository {
    override fun create(
        ...
        return db.blockingQuery {
            ...
            val projectsMap = mutableMapOf<Identifier, ProjectDao>()

            projects.forEach { projectsMap[it.identifier] = createProject(analyzerRun, it) }

            packages.forEach {
                val pkgDao = createPackage(analyzerRun, it)

                shortestDependencyPaths[it.identifier]?.onEach { shortestDependencyPath ->
