 ...
 * by and the IDS to be included. The function returns an [Op] representing the condition. The conditions for the
 * different hierarchy levels are then combined using an `OR` operator.
 */
typealias HierarchyConditionGenerator = SqlExpressionBuilder.(Int, List<CompoundHierarchyId>) -> Op<Boolean>
...
fun HierarchyFilter.apply(
    ...
    if (isWildcard) {
        ...
        val hierarchyCondition = elementsByLevel.entries.fold(Op.FALSE as Op<Boolean>) { op, (level, ids) ->
            val condition = generator(this, level, ids)
            op or condition
        }
