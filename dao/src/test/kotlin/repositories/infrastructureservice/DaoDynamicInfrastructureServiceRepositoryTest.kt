...
class DaoDynamicInfrastructureServiceRepositoryTest : WordSpec() {
    private val dbExtension = extension(DatabaseTestExtension())

    private lateinit var infrastructureServicesRepository: DaoInfrastructureServiceRepository
    private lateinit var dynamicInfrastructureServicesRepository: DaoDynamicInfrastructureServiceRepository
    ...
    init {
        beforeEach {
            ...
            dynamicInfrastructureServicesRepository = dbExtension.fixtures.dynamicInfrastructureServiceRepository
        ...
        "getOrCreateForRun" should {
            "create a new entity in the database" {
                val expectedService = createDynamicInfrastructureService()

                val service = dynamicInfrastructureServicesRepository.getOrCreateForRun(
                    expectedService,
                    fixtures.ortRun.id
                )
                ...
                val runServices = dynamicInfrastructureServicesRepository.listForRun(fixtures.ortRun.id)
            ...
            "reuse an already existing entity" {
                ...
                val serviceForOtherRun =
                    dynamicInfrastructureServicesRepository.getOrCreateForRun(expectedService, otherRun.id)

                val serviceForRun =
                    dynamicInfrastructureServicesRepository.getOrCreateForRun(expectedService, fixtures.ortRun.id)
            ...
            "not reuse a service assigned to an organization" {
                ...
                val runService = createDynamicInfrastructureService()
                val dbRunService = dynamicInfrastructureServicesRepository.getOrCreateForRun(
                    runService,
                    fixtures.ortRun.id
                )
            ...
            "throw exception if the entity name is invalid" {
                ...
                val newService = createDynamicInfrastructureService(name = serviceName)

                val exception = shouldThrow<ValidationException> {
                    dynamicInfrastructureServicesRepository.getOrCreateForRun(newService, fixtures.ortRun.id)
                }

                exception.message shouldInclude serviceName

                dynamicInfrastructureServicesRepository.listForRun(fixtures.ortRun.id) shouldBe emptyList()
            }
        }

        "listForRun" should {
            "return all services assigned to a run" {
                val runService1 = createDynamicInfrastructureService(name = "run1")
                val runService2 = createDynamicInfrastructureService(name = "run2")
                ...
                infrastructureServicesRepository.create(orgService)
                infrastructureServicesRepository.create(prodService)
                dynamicInfrastructureServicesRepository.getOrCreateForRun(runService1, fixtures.ortRun.id)
                dynamicInfrastructureServicesRepository.getOrCreateForRun(runService2, fixtures.ortRun.id)
    ...
    private fun createDynamicInfrastructureService(
        name: String = SERVICE_NAME,
        url: String = SERVICE_URL,
        usernameSecret: Secret = this.usernameSecret,
        passwordSecret: Secret = this.passwordSecret,
