...
class DaoRepositoryRepositoryTest : StringSpec({
    ...
    "list should filter with example com domain pattern" {
        ...
        val repo1 = fixtures.createRepository(url = "https://example.com/auth-repository.git") // Should match
        val repo2 = fixtures.createRepository(url = "https://example.com/core-service.git") // Should match
        fixtures.createRepository(url = "https://github.com/user/repo.git") // Should not match (different domain)

        val repo4 = fixtures.createRepository(
            url = "https://subdomain.example.com/repo.git"
        ) // Should match (subdomain of example.com)

        // Test regex: URLs containing example.com domain
