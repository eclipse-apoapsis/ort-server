...
class DaoOrganizationRepositoryTest : StringSpec({
    ...
    "list should filter organizations using basic regex pattern" {
        ...
        organizationRepository.create("orange-company", "description3") // Should not match
        ...
        // Test regex pattern: names containing "org"
    ...
    "list should filter with regex ending pattern" {
        ...
        organizationRepository.create("api-gateway", "Gateway service") // Should not match
        val createdOrg2 = organizationRepository.create("auth-api", "Authentication API")
        organizationRepository.create("core-service", "Core service") // Should not match

        val parameters = ListQueryParameters(limit = 10)

        // Test regex: names ending with "api"
    ...
    "list should filter with regex starting pattern" {
        ...
        organizationRepository.create("user-core", "User core") // Should not match
        organizationRepository.create("api-service", "API service") // Should not match

        val parameters = ListQueryParameters(limit = 10)

        // Test regex: names starting with "core"
