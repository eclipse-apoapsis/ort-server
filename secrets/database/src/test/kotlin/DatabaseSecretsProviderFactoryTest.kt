...
class DatabaseSecretsProviderFactoryTest : StringSpec({
    extension(DatabaseTestExtension())

    "A SecretStorage accessing the database provider can be created" {
        val storage = SecretStorage.createStorage(ConfigManager.create(ConfigFactory.parseMap(secretsProviderConfig)))
