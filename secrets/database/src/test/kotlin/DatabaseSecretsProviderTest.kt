...
class DatabaseSecretsProviderTest : WordSpec({
    ...
    "writeSecret" should {
        ...
        "persist encrypted values instead of plaintext" {
            ...
            row[DatabaseSecretsTable.encryptedValue].contains(plaintext) shouldBe false
        }
...
private fun createStorage(): SecretStorage {
        val config = ConfigFactory.parseMap(secretsProviderConfig)

        return SecretStorage.createStorage(ConfigManager.create(config))
}

internal val secretsProviderConfig: Map<String, Any> = mapOf(
    "secretsProvider.name" to "database",
    "secretsProvider.databaseMasterPassword" to "thisIsAStrongPassword",
    "secretsProvider.databaseSalt" to "deadbeefcafebabedeadbeefcafebabe",
    "secretsProvider.databaseKeyVersion" to 1
)
