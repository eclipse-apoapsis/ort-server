...
internal data class DatabaseSecretsConfiguration(
    ...
    companion object {
        ...
        fun create(configManager: ConfigManager): DatabaseSecretsConfiguration {
            val masterPassword = configManager.getSecret(Path(MASTER_PASSWORD_PROPERTY)).trim()
            validateMasterPassword(masterPassword)

            val salt = configManager.getString(SALT_PROPERTY).trim()
            validateSalt(salt)
        ...
        private fun validateMasterPassword(masterPassword: String) {
            ...
            require(masterPassword.length >= MINIMUM_MASTER_PASSWORD_LENGTH) {
...
private inline fun require(condition: Boolean, lazyMessage: () -> String) {
