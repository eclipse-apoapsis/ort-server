...
class DefaultKeycloakClient(
    ...
    override suspend fun addUserToGroup(username: UserName, groupName: GroupName) {
        // FIXME: Unfortunately, no way to distinguish if there was a technical problem or the user was not found.
        val user = try { getUser(username) } catch (e: KeycloakClientException) {
            throw UserNotFoundException("User with username '${username.value}' not found.", e)
        }
    ...
    override suspend fun removeUserFromGroup(username: UserName, groupName: GroupName) {
        ...
        val user = try {
            getUser(username)
        } catch (e: KeycloakClientException) {
            throw UserNotFoundException("User with username '${username.value}' not found.", e)
        }
        val group = getGroup(groupName)
