...
class RepositoriesRouteIntegrationTest : AbstractIntegrationTest({
    ...
    "POST /repositories/{repositoryId}/runs" should {
        ...
        keepAliveTestCases.forEach { testCase ->
            "respond with 'Forbidden' if 'keepAliveWorker' is true in the ${testCase.workerName} config " +
                    "(non super-user calling)" {
                integrationTestApplication {
            ...
            "respond with 'Forbidden' if '${testCase.parameterName}' is defined in configuration parameters " +
                    "(non super-user calling)" {
                integrationTestApplication {
                    val repositoryId = createRepository().id
        ...
        "continue if 'keepAliveWorker' is true in the Analyzer config (super-user calling)" {
            integrationTestApplication {
                val repositoryId = createRepository().id
        ...
        "continue if 'analyzerKeepAlive' is defined in configuration parameters (super-user calling)" {
            integrationTestApplication {
                val repositoryId = createRepository().id
                ...
                val createRun = CreateOrtRun(
                    ...
                    null,
                    ApiJobConfigurations(
                        parameters = mapOf("analyzerKeepAlive" to "true")
                    )
                )
