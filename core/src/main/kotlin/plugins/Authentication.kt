...
fun Application.configureAuthentication() {
    ...
    val roleCache = Caffeine.newBuilder().expireAfterWrite(1.minutes).asCache<String, Set<String>>()
...
internal suspend fun getRoles(
    ...
    roleCache.get(subject) {
        ...
        result.onSuccess {
            logger.debug("Loaded ${it.size} Keycloak roles for '$subject' in $duration.")
        }.onFailure {
            logger.error("Failed to load Keycloak roles for '$subject'.", it)
        }.getOrThrow()
    }
