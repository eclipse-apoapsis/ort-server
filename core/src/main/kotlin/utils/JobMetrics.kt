...
class JobMetrics(private val application: Application) : MeterBinder {
    ...
    private fun getAnalyzerJobDurations(timestamp: Instant): List<Duration> =
        transaction {
            AnalyzerJobsTable.selectAll().where {
            ...
            }.map { it[AnalyzerJobsTable.finishedAt]!! - it[AnalyzerJobsTable.startedAt]!! }
        }

    private fun getAdvisorJobDurations(timestamp: Instant): List<Duration> =
        transaction {
            AdvisorJobsTable.selectAll().where {
            ...
            }.map { it[AdvisorJobsTable.finishedAt]!! - it[AdvisorJobsTable.startedAt]!! }
        }

    private fun getEvaluatorJobDurations(timestamp: Instant): List<Duration> =
        transaction {
            EvaluatorJobsTable.selectAll().where {
            ...
            }.map { it[EvaluatorJobsTable.finishedAt]!! - it[EvaluatorJobsTable.startedAt]!! }
        }

    private fun getScannerJobDurations(timestamp: Instant): List<Duration> =
        transaction {
            ScannerJobsTable.selectAll().where {
            ...
            }.map { it[ScannerJobsTable.finishedAt]!! - it[ScannerJobsTable.startedAt]!! }
        }

    private fun getReporterJobDurations(timestamp: Instant): List<Duration> =
        transaction {
            ReporterJobsTable.selectAll().where {
            ...
            }.map { it[ReporterJobsTable.finishedAt]!! - it[ReporterJobsTable.startedAt]!! }
        }
}
