 ...
 * [PluginConfig]s. Some job configs allow to enable plugins without addintional configuration, in this case `null` is
 * used as the value for the plugin ID in the map.
 */
fun CreateOrtRun.getPluginConfigs(): Map<PluginType, Map<String, PluginConfig?>> =
    buildMap {
        jobConfigs.advisor?.let { jobConfig ->
            ...
            put(PluginType.ADVISOR, allAdvisors.associateWith { jobConfig.config?.get(it) })
        }
        ...
        jobConfigs.scanner?.let { jobConfig ->
            ...
            put(PluginType.SCANNER, allScanners.associateWith { jobConfig.config?.get(it) })
        }
        ...
        jobConfigs.reporter?.let { jobConfig ->
            ...
            put(PluginType.REPORTER, allReporters.associateWith { jobConfig.config?.get(it) })
