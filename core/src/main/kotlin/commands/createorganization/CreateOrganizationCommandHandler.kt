...
class CreateOrganizationCommandHandler(private val organizationService: OrganizationService) :
    CommandHandler<CreateOrganizationCommand, Long> {
    override suspend fun execute(command: CreateOrganizationCommand): Long {
        return organizationService.createOrganization(command.name, command.description).id
    }
}
