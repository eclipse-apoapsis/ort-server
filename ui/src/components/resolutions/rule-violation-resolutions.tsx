...
import {
  deleteRuleViolationResolutionForRepositoryMutation,
...
type RuleViolationResolutionsProps = {
  item: Extract<ItemWithResolutions, { rule: unknown }>;
  repositoryId: string;
  runId: number;
};

export function RuleViolationResolutions({
  item,
  repositoryId,
  runId,
}: RuleViolationResolutionsProps) {
  const [editingKey, setEditingKey] = useState<string | null>(null);
  const queryClient = useQueryClient();
  const createContext: ManagedResolutionContext = {
    itemType: 'rule-violation',
    identifier: item.message,
    repositoryId,
    runId,
  };
  const displayItems = getDisplayItems(item, true);
  const hasAnyResolution = hasRuleViolationResolutionActivity(item);

  const { mutateAsync: deleteResolution } = useMutation({
    ...deleteRuleViolationResolutionForRepositoryMutation(),
    onSuccess() {
      queryClient.invalidateQueries({
        queryKey: getRunRuleViolationsQueryKey({ path: { runId } }),
      });
      toast.info('Resolution deleted', {
        description: 'Rule violation resolution deleted successfully.',
      });
    },
