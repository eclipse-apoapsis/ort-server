...
import {
  deleteProductInfrastructureServiceMutation,
  getProductInfrastructureServicesOptions,
  getProductInfrastructureServicesQueryKey,
  getProductOptions,
  ...
  const { mutateAsync: delService } = useMutation({
    ...
    onSuccess() {
      ...
      queryClient.invalidateQueries({
        queryKey: getProductInfrastructureServicesQueryKey({
    ...
    <div className='flex justify-end gap-1'>
      <Tooltip>
        <TooltipTrigger asChild>
          <Link
            to='/organizations/$orgId/products/$productId/infrastructure-services/$serviceName/edit'
            params={{
              orgId: params.orgId,
...
const InfrastructureServices = () => {
  ...
  if (productIsError || infraIsError) {
    toastError('Unable to load data', productError || infraError);
    return;
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Manage infrastructure services for {product.name}
        </CardDescription>
        <div className='py-2'>
          <Tooltip>
            <TooltipTrigger asChild>
            ...
            <TooltipContent>
              Create a new infrastructure service for this product
            </TooltipContent>
          </Tooltip>
        </div>
      </CardHeader>
