...
export function formValuesToPayload(
  ...
  //     Object.entries(config).filter(([key]) => advisors.includes(key))
  //   );
  //   return Object.keys(filtered).length > 0 ? (filtered as { [key: string]: PluginConfig }) : undefined;
  // }
  ...
  const advisorConfig = values.jobConfigs.advisor.enabled
    ? {
        ...
        // config: values.jobConfigs.advisor.config
        //   ? Object.fromEntries(
        //       Object.entries(values.jobConfigs.advisor.config).filter(([key]) =>
        //         values.jobConfigs.advisor.advisors.includes(key)
        //       )
        //     ) as {
