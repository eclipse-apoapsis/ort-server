...
import { DetectedLicense, DetectedLicensePackage } from '@/api';
...
const packageColumnHelper = createColumnHelper<DetectedLicensePackage>();
...
const PackageIdCell = ({
  ...
  pkg: DetectedLicensePackage;
  packageIdType: PackageIdType;
}) => {
  const id =
    packageIdType === 'PURL' && pkg.purl ? pkg.purl : pkg.ortIdentifier;
...
export const DetectedLicensePackagesTable = ({
  ...
  const packageColumnId = packageIdType === 'PURL' ? 'purl' : 'ortIdentifier';
      ...
      <DataTable
        ...
        renderSubComponent={({ row: packageRow }) => (
          <DetectedLicenseFindingsTable
            runId={runId}
            license={row.original.license}
            identifier={packageRow.original.ortIdentifier}
          />
        )}
