...
class ReporterRunner(
    ...
    /**
     * The service to access the admin configuration. Some parts of the report configuration is obtained from there.
     */
            ...
            val success = config.formats.map { format ->
                async {
                    ...
                    result.getOrNull()?.let { (_, reportFiles) ->
                        val nameMapper = ReportNameMapper.create(adminConfig.reportDefinitions.getValue(format))
...
private fun fetchReporterFactory(format: String, adminConfig: ReporterConfig): ReporterFactory {
    val pluginId = requireNotNull(adminConfig.reportDefinitions[format]?.pluginId) {
