...
internal class AnalyzerWorker(
    ...
    suspend fun run(jobId: Long, traceId: String): RunResult = runCatching {
        ...
        analyzerRun.result.projects.forEach { project ->
            val identifierToShortestPathsForProject = getIdentifierToShortestPathsMap(
                project.id.mapToModel(),
                ortResult.dependencyNavigator.getShortestPaths(project)
            )

            identifierToShortestPathsForProject.forEach { (identifier, path) ->
                val existingEntry = shortestPathsByIdentifier[identifier]

                if (existingEntry != null) {
                    existingEntry.add(path)
                } else {
                    shortestPathsByIdentifier[identifier] = mutableListOf(path)
                }
            }
        }
