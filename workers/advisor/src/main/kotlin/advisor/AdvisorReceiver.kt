...
class AdvisorReceiver(private val config: Config) {
    fun receive(handler: (advisorJobId: Long, traceId: String) -> RunResult) {
        ...
        MessageReceiverFactory.createReceiver(AdvisorEndpoint, config) { message ->
            ...
            val jobId = message.payload.advisorJobId

            val response = when (val result = handler(jobId, traceId)) {
                is RunResult.Success -> {
                    logger.info("Advisor job '$jobId' succeeded.")
                    Message(MessageHeader(token, traceId), AdvisorWorkerResult(jobId))
                }

                is RunResult.Failed -> {
                    logger.error("Advisor job '$jobId' failed.", result.error)
                    Message(MessageHeader(token, traceId), AdvisorWorkerError(jobId))
                }
