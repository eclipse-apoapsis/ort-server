...
class EnvironmentServiceTest : WordSpec({
    ...
    "setUpEnvironment from a file" should {
        "invoke all generators to produce the supported configuration files" {
            ...
            val dynamicServiceRepository = mockk<DynamicInfrastructureServiceRepository>()
        ...
        "associate all infrastructure services from the config file with the current ORT run" {
            ...
            val expectedDynamicServices = services.map { it.mapToDynamicInfrastructureService() }
        ...
        "assign the infrastructure services for the repository to the current ORT run" {
            ...
            val expectedDynamicServices = listOf(repositoryService, otherService).map(
                InfrastructureService::mapToDynamicInfrastructureService
            )
        ...
        "set an overridden credentials type when assigning infrastructure services to the current ORT run" {
            ...
            val expectedAssignedService = service.copy(
                credentialsTypes = EnumSet.of(CredentialsType.GIT_CREDENTIALS_FILE)
            ).mapToDynamicInfrastructureService()
    ...
    "setupAuthenticationForCurrentRun" should {
        "setup the authenticator with services stored in the database" {
            ...
            val dynamicServices = services.map { it.mapToDynamicInfrastructureService() }
...
/**
 * Prepare this mock for an [DynamicInfrastructureServiceRepository] to expect calls that assign infrastructure services
 * to the current ORT run. Return a list that contains the assigned services after running the test.
 */
private fun DynamicInfrastructureServiceRepository.expectServiceAssignments(): List<DynamicInfrastructureService> {
    val assignedServices = mutableListOf<DynamicInfrastructureService>()

    val slotService = slot<DynamicInfrastructureService>()
    every { getOrCreateForRun(capture(slotService), RUN_ID) } answers {
        firstArg<DynamicInfrastructureService>().also { service ->
