 ...
 * model used by this class is a logic file system that is identified by a [ResolvedConfigContext] object. In this file
 * system, the desired configuration files can be selected via instances of the [Path] class.
 */
interface ConfigFileProvider {
    /**
     * Resolve a [RequestedConfigContext] to a [ResolvedConfigContext]. This function is useful for contexts that can
     * change over time, for instance, if configuration files are stored in a VCS in a specific branch. Throw an
     * exception if the context cannot be resolved.
     *
