...
class LicenseFindingService(private val db: Database) {
    fun getRunDetectedLicenses(
        ortRunId: Long,
        parameters: ListQueryParameters,
        licenseFilter: String?
    ): ListQueryResult<DetectedLicense> = db.blockingQuery {
        // includeAnalyzerData = false: skips the 4 LEFT JOINs to analyzer tables that are only
        // needed for purl retrieval. Including them causes row multiplication via packages_analyzer_runs.
    ...
    fun getRunPackagesWithDetectedLicense(
        ortRunId: Long,
        license: String,
        parameters: ListQueryParameters,
        identifierFilter: String?,
        purlFilter: String?
    ): ListQueryResult<DetectedLicensePackage> = db.blockingQuery {
        ...
        ListQueryResult(
            data = rows.map { row ->
                DetectedLicensePackage(
                    ortIdentifier = buildOrtIdentifier(row),
    ...
    fun getRunDetectedLicenseFindings(
        ortRunId: Long,
        license: String,
        identifier: String,
        parameters: ListQueryParameters
    ): ListQueryResult<LicenseFindingDetail> = db.blockingQuery {
        val ctx = buildQueryContext(includeAnalyzerData = false)
        ...
        val limit = parameters.limit ?: ListQueryParameters.DEFAULT_LIMIT
        val offset = parameters.offset ?: 0L
        val rows = query.limit(limit).offset(offset).toList()

        ListQueryResult(
            data = rows.map { row ->
                LicenseFindingDetail(
...
// includeAnalyzerData controls whether the 4 LEFT JOINs for purl lookup are included. Set to false
// when purl is not needed, as packages_analyzer_runs is a junction table that multiplies rows by the
// number of analyzer runs a package has been part of, forcing expensive COUNT(DISTINCT) to compensate.
...
private fun buildOrtIdentifier(row: ResultRow): String =
    buildString {
        append(row[IdentifiersTable.type])
        append(':')
        append(row[IdentifiersTable.namespace])
        append(':')
        append(row[IdentifiersTable.name])
        append(':')
        append(row[IdentifiersTable.version])
    }
