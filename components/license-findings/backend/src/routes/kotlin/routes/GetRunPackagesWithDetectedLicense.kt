...
internal fun Route.getRunPackagesWithDetectedLicense(
    ...
    response {
        HttpStatusCode.OK to {
            description = "Success."
            jsonBody<PagedResponse<DetectedLicensePackage>> {
                example("Get packages with detected license") {
                    value = PagedResponse(
                        data = listOf(
                            DetectedLicensePackage(
                                "Maven:com.example:lib:1.0",
    ...
    val result = service.getRunPackagesWithDetectedLicense(
        ...
        parameters = call.pagingOptions(SortProperty("ortIdentifier", SortDirection.ASCENDING)).mapToModel(),
