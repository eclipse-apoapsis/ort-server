    ...
    "GetRunDetectedLicenses" should {
        "require RepositoryPermission.READ_ORT_RUNS" {
            requestShouldRequireRole(
                routes = {
                    route("runs/{runId}") {
                        licenseFindingRoutes(service, requireRunPermission(ortRunRepository))
                    }
                },
                role = RepositoryRole.READER,
                hierarchyId = hierarchyId
            ) {
                get("/runs/$ortRunId/license-findings/licenses")
            }
        }
    }
    ...
    "GetRunPackagesWithDetectedLicense" should {
        "require RepositoryPermission.READ_ORT_RUNS" {
            requestShouldRequireRole(
                ...
                get("/runs/$ortRunId/license-findings/licenses/Apache-2.0/packages")
            }
        }
    }
    ...
    "GetRunDetectedLicenseFindings" should {
        "require RepositoryPermission.READ_ORT_RUNS" {
            ...
            requestShouldRequireRole(
                ...
                get("/runs/$ortRunId/license-findings/licenses/Apache-2.0/packages/$encodedIdentifier/findings")
            }
        }
    }
})
    ...
    object : AuthorizationChecker {
        override suspend fun loadEffectiveRole(
            ...
            val ortRun = ortRunRepository.get(runId)
                ?: throw InvalidHierarchyIdException(RepositoryId(runId))
