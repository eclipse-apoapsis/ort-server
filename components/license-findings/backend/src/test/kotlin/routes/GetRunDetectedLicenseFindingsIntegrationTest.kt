    ...
    "GetRunDetectedLicenseFindings" should {
        "return 200 with file-level findings sorted by path" {
            licenseFindingTestApplication { client ->
                ...
                val response = client.get(
                    "/runs/${seeded.ortRunId}/license-findings/licenses/Apache-2.0/packages/$encodedIdentifier/findings"
                )

                response.status shouldBe HttpStatusCode.OK
                response.body<PagedResponse<LicenseFindingDetail>>() shouldBe PagedResponse(
                    data = listOf(
        ...
        "return 200 with paginated findings when limit and offset are provided" {
            licenseFindingTestApplication { client ->
                ...
                val response = client.get(
                    "/runs/${seeded.ortRunId}/license-findings/licenses/Apache-2.0/packages/" +
                            "$encodedIdentifier/findings?limit=1&offset=1"
                )

                response.status shouldBe HttpStatusCode.OK
                response.body<PagedResponse<LicenseFindingDetail>>() shouldBe PagedResponse(
                    data = listOf(LicenseFindingDetail("LICENSE", 1, 10, 99f, "LicenseScanner 1.0.0")),
                    pagination = PagingData(
                        limit = 1,
                        offset = 1,
                        totalCount = 2,
        ...
        "return 200 with an empty list for an unknown license" {
            licenseFindingTestApplication { client ->
                ...
                val response = client.get(
                    "/runs/${seeded.ortRunId}/license-findings/licenses/Zlib/packages/$encodedIdentifier/findings"
                )
