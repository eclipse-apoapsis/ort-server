    ...
    "GetRunDetectedLicenses" should {
        "return 200 with all licenses and package counts sorted by license" {
            licenseFindingTestApplication { client ->
                val response = client.get("/runs/$ortRunId/license-findings/licenses")
        ...
        "filter by the license query parameter case-insensitively" {
            licenseFindingTestApplication { client ->
                val response = client.get("/runs/$ortRunId/license-findings/licenses?license=apache")
        ...
        "return 200 with empty data for an unknown run" {
            licenseFindingTestApplication { client ->
                val response = client.get("/runs/999999/license-findings/licenses")
