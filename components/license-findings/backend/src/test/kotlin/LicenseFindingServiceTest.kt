    ...
    init {
        ...
        "getRunDetectedLicenses" should {
            "return all licenses with package counts sorted by license ascending" {
                val seed = seedData(fixtures, db)

                val result = service.getRunDetectedLicenses(
                    seed.ortRunId,
            ...
            "count each package only once even with duplicate package rows" {
                val seed = seedData(fixtures, db, duplicatePackageEntries = true)
        ...
        "getRunPackagesWithDetectedLicense" should {
            "return the package that has the given license" {
                ...
                val result = service.getRunPackagesWithDetectedLicense(
                    seed.ortRunId,
                    "MIT",
                    ListQueryParameters(sortFields = listOf(OrderField("ortIdentifier", OrderDirection.ASCENDING))),
                    null,
                    null
                )
                ...
                result.totalCount shouldBe 1
                result.data shouldContainExactly listOf(
                    DetectedLicensePackage(seed.vcsIdentifier.toCoordinates(), seed.vcsPurl)
                )
            }
            ...
            "apply identifierFilter ILIKE" {
                ...
                result.totalCount shouldBe 1
                result.data shouldContainExactly listOf(
                    DetectedLicensePackage(seed.artifactIdentifier.toCoordinates(), seed.artifactPurl)
                )
            }
            ...
            "respect limit and offset" {
                ...
                val result = service.getRunPackagesWithDetectedLicense(
                    ...
                    "Apache-2.0",
                    ListQueryParameters(
                        sortFields = listOf(OrderField("ortIdentifier", OrderDirection.ASCENDING)),
                        limit = 1,
                        offset = 1
                    ),
                    null,
        ...
        "getRunDetectedLicenseFindings" should {
            "return findings for the given license and package sorted by path" {
                ...
                result.totalCount shouldBe 2
                result.data shouldContainExactly listOf(
                    LicenseFindingDetail("docs/NOTICE.Apache", 11, 18, 87.5f, "LicenseScanner 1.0.0"),
                    LicenseFindingDetail("LICENSE", 1, 10, 99f, "LicenseScanner 1.0.0")
                )
            }
            ...
            "return findings with null score when score is absent" {
                ...
                result.totalCount shouldBe 1
                result.data shouldContainExactly listOf(
                    LicenseFindingDetail("NOTICE", 2, 3, null, "LicenseScanner 1.0.0")
                )
            }
            ...
            "return empty list for an unknown license" {
                ...
                service.getRunDetectedLicenseFindings(
                    seed.ortRunId,
                    "NOASSERTION",
                    seed.artifactIdentifier.toCoordinates(),
                    ListQueryParameters(sortFields = listOf(OrderField("path", OrderDirection.ASCENDING)))
...
private fun Identifier.toCoordinates(): String = "$type:$namespace:$name:$version"
