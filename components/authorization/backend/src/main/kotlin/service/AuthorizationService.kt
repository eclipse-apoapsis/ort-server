...
interface AuthorizationService {
    ...
    /**
     * Return a [Set] with the [CompoundHierarchyId]s of all hierarchy elements for which the specified [userId] has at
     * least all the permissions defined by [organizationPermissions], [productPermissions], and
     * [repositoryPermissions]. With the optional [containedIn] ID, the list can be restricted to elements that belong
     * to this root element (directly or indirectly). This function can be used to generate filters for queries based
     * on the access rights of a user. For the returned [CompoundHierarchyId]s not necessarily all components
     ...
     */
    suspend fun listAccessibleHierarchyIds(
        userId: String,
