    ...
    suspend fun findOrtRunsByPackage(
        ...
        return db.blockingQuery {
            ...
            val scopeCondition = when (scope) {
                is OrganizationId -> ProductsTable.organizationId eq scope.value
                is ProductId -> RepositoriesTable.productId eq scope.value
                is RepositoryId -> OrtRunsTable.repositoryId eq scope.value
                else -> Op.TRUE
            }

            val baseCondition = concatenatedIdentifier.applyRegex(identifier) and scopeCondition

            val whereClause = hierarchyFilter.apply(baseCondition) { level, ids, filter ->
