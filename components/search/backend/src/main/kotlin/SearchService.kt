...
class SearchService(
    ...
    suspend fun findOrtRunsByPackage(
        ...
        return db.blockingQuery {
            ...
            when {
                purl != null -> findByPurl(query, purl, hierarchyFilter)
                // Using checkNotNull instead of !! to satisfy detekt's UnsafeCallOnNullableType rule.
                // The require check above guarantees identifier is non-null when purl is null.
                else -> findByIdentifier(query, checkNotNull(identifier), hierarchyFilter)
            }
        }
    }

    private fun createBaseQuery() = OrtRunsTable
    ...
    /**
     * Search by PURL using curated PURL resolution.
     * The effective PURL is determined by COALESCE(curated_purl, original_purl).
     */
    ...
    /**
     * Create an expression for the effective PURL that respects curations.
     * Returns COALESCE(curated_purl, original_purl) where the curated PURL is looked up
     * from the resolved configuration for the current ORT run.
     */
    ...
    /**
     * Search by ORT identifier without curation resolution (for performance).
     */
