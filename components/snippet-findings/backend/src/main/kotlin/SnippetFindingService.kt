...
class SnippetFindingService(private val db: Database) {
    ...
    fun getProvenancesForRun(
        ...
        val merged = LinkedHashMap<Long, SnippetFindingProvenance>()
        (directProvenances + nestedProvenances).forEach { merged.putIfAbsent(it.id, it) }

        // Attach snippet finding counts so callers can see how many findings each provenance has.
        val counts = countSnippetFindingsByProvenanceId(merged.keys.toList())
        var result = merged.values.map { it.copy(snippetFindingCount = counts[it.id] ?: 0L) }
    ...
    fun getSnippetFindingsForRun(
        ...
        val query = buildQueryContext().join
            .join(
 ...
 * works for both root-package scan results and nested sub-repository scan results, unlike the old path through
 * [ScanResultPackageProvenancesTable] which excluded nested scan results.
 */
private fun buildQueryContext(): QueryContext {
    val join = SnippetFindingsTable
        .innerJoin(ScanSummariesTable)
        .join(ScanResultsTable, JoinType.INNER, ScanSummariesTable.id, ScanResultsTable.scanSummaryId)
        .join(
