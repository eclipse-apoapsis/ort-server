    ...
    "RemoveTemplateFromOrganization" should {
        "remove a template from an organization if it exists" {
            integrationTestApplication { client ->
                ...
                pluginTemplateService.addOrganization("template1", pluginType, pluginId, 2, "test-user")
                ...
                template.organizationIds.orEmpty() should containExactly(2)
            }
        }
        ...
        "return NotFound if the organization does not exist" {
            integrationTestApplication { client ->
                ...
                client.post(
                    "/admin/plugins/$pluginType/$pluginId/templates/template1/removeFromOrganization?organizationId=999"
                ) shouldHaveStatus HttpStatusCode.NotFound
            }
        }
        ...
        "normalize the plugin ID" {
            integrationTestApplication { client ->
                ...
                val template = result.get().shouldNotBeNull()
                template.organizationIds.orEmpty() should beEmpty()
            }
        }
    }
})
