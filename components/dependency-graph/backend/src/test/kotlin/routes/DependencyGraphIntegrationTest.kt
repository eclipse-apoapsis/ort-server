...
abstract class DependencyGraphIntegrationTest(
    body: DependencyGraphIntegrationTest.() -> Unit
    ...
    fun dependencyGraphTestApplication(
        block: suspend ApplicationTestBuilder.(client: HttpClient) -> Unit
    ) = integrationTestApplication(
        routes = {
            route("runs/{runId}") {
                dependencyGraphRoutes(service, requirePermission(RepositoryPermission.READ))
            }
        },
        block = block
    )
