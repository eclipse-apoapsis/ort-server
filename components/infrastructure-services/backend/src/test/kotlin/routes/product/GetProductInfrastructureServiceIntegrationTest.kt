...
class GetProductInfrastructureServiceIntegrationTest : InfrastructureServicesIntegrationTest({
    "GetProductInfrastructureService" should {
        "return an infrastructure service" {
            infrastructureServicesTestApplication { client ->
                ...
                val response = client.get(
                    "/products/$prodId/infrastructure-services/${service.name}"
                )
        ...
        "respond with 'NotFound' if no infrastructure service exists" {
            infrastructureServicesTestApplication { client ->
                val response = client.get("/products/$prodId/infrastructure-services/not-existing-service")
