...
internal fun Route.patchRuleViolationResolution(
    ...
    {
        operationId = "patchRuleViolationResolutionForRepository"
        summary = "Update a rule violation resolution for a repository."
        description = "Update a rule violation resolution for a repository."
        ...
        request {
            ...
            pathParameter<String>("messageHash") {
                description = "The stable technical identifier of the rule violation resolution to update."
                required = true
            }

            jsonBody<PatchRuleViolationResolution> {
                description = "The details of the rule violation resolution to update."
                required = true

                example("Example") {
                    value = PatchRuleViolationResolution(
        ...
        response {
            HttpStatusCode.OK to {
                description = "The rule violation resolution was updated successfully."
            }
    ...
    val patchRuleViolationResolution = call.receive<PatchRuleViolationResolution>()

    ruleViolationResolutionService.updateResolutionByHash(
        repositoryId = RepositoryId(repositoryId),
        messageHash = messageHash,
        updatedBy = userId,
        reason = patchRuleViolationResolution.reason?.mapToModel(),
        comment = patchRuleViolationResolution.comment
    ).onOk {
