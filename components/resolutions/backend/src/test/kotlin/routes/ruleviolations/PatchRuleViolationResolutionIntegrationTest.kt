...
class PatchRuleViolationResolutionIntegrationTest : ResolutionsIntegrationTest({
    ...
    "PatchRuleViolationResolution" should {
        "update a rule violation resolution for a repository" {
                ...
                val response = client.patch(
                    ...
                    setBody(
                        PatchRuleViolationResolution(
                            ...
                            reason = ApiRuleViolationResolutionReason.DYNAMIC_LINKAGE_EXCEPTION
                        )
                    )
                }
        ...
        "fail if the repository does not exist" {
            resolutionsTestApplication { client ->
                val response = client.patch(
                    "/repositories/9999/resolutions/rule-violations/" +
                            calculateResolutionMessageHash("A rule violation message.")
                ) {
                    setBody(
                        PatchRuleViolationResolution(
        ...
        "fail if the rule violation resolution does not exist" {
            resolutionsTestApplication { client ->
                val response = client.patch(
                    "/repositories/${repositoryId.value}/resolutions/rule-violations/missing-rule-violation-hash"
                ) {
                    setBody(
                        PatchRuleViolationResolution(
                            comment = "This rule violation is accepted because of dynamic linking.",
