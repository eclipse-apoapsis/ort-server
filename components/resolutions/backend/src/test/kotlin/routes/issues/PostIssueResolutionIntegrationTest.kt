...
class PostIssueResolutionIntegrationTest : ResolutionsIntegrationTest({
    ...
    "PostIssueResolution" should {
        "create an issue resolution for a repository" {
            resolutionsTestApplication { client ->
                val response = client.post("/repositories/${repositoryId.value}/resolutions/issues") {
                    setBody(
                        PostIssueResolution(
                            message = "scanner-issue",
                            comment = "This scanner issue is a known false positive.",
                            reason = ApiIssueResolutionReason.SCANNER_ISSUE
                        )
                    )
                }
        ...
        "fail if the repository does not exist" {
            resolutionsTestApplication { client ->
                val response = client.post("/repositories/9999/resolutions/issues") {
                    setBody(
                        PostIssueResolution(
                            message = "scanner-issue",
                            comment = "This scanner issue is a known false positive.",
        ...
        "fail if there is already a resolution for the issue" {
            resolutionsTestApplication { client ->
